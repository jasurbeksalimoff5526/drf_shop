...
from .models import CustomUser, CodeVerify, VIA_EMAIL, VIA_PHONE, CHANGE_INFO, CODE_VERIFY, DONE
...
class ResendCodeSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if active_code:
            remaining = active_code.expiration_time - timezone.now()

            raise ValidationError({
                "message": f"Sizda hali aktiv kod mavjud. {remaining.seconds // 60} daqiqa \
{remaining.seconds % 60 } soniyadan dan keyin urinib ko'ring."
            })
        attrs["user"] = user
        return attrs
...
class ChangeProfileInfoSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if CustomUser.objects.exclude(
            ...
            raise ValidationError({"email_or_phone": "Bu username allaqachon ro'yxatdan o'tgan!"})
        return attrs
...
class ProfileSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = CustomUser
        fields = ["id", "username", "first_name", "last_name", "email", "phone_number", "photo", "user_role"]
...
class ForgotPasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if active_code:
            remaining = int(
                (active_code.expiration_time - timezone.now()).total_seconds()
            )
            minutes = remaining // 60
            seconds = remaining % 60
            raise ValidationError({
                "message": f"Aktiv kod mavjud. {minutes} daqiqa, {seconds} soniyadan keyin qayta urinib ko'ring."
            })
...
class ResetPasswordSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        if not verify_code:
            raise ValidationError({"code": "Tasdiqlash kodi xato yoki muddati tugagan!"})
